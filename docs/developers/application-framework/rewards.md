...
[tmpdoc](https://developers.komodoplatform.com/basic-docs/cryptoconditions/cc-rewards.html)

[code @ github](https://github.com/KomodoPlatform/komodo/blob/master/src/cc/rewards.cpp)
...
* Anyone can create a new plan using [rewardscreatefunding](../../basic-docs/cryptoconditions/cc-rewards.html#rewardscreatefunding)
* Anyone can add funding to the plan using [rewardsaddfunding](../../basic-docs/cryptoconditions/cc-rewards.html#rewardsaddfunding)
* Anyone can query the list of all active plans using [rewardslist](../../basic-docs/cryptoconditions/cc-rewards.html#rewardslist)
* To get the details of a particular plan, use [rewardsinfo](../../basic-docs/cryptoconditions/cc-rewards.html#rewardsinfo)
* After finding a suitable plan, any user can lock funds using [rewardslock](../../basic-docs/cryptoconditions/cc-rewards.html#rewardslock)
* After the minimum lock time is met, the user can use [rewardsunlock](../../basic-docs/cryptoconditions/cc-rewards.html#rewardsunlock) to unlock their funds and receive their rewards
...
Use [rewardslist](../../basic-docs/cryptoconditions/cc-rewards.html#rewardslist) to see a list of all available `fundingtxid`'s.
...
The method returns a `hex` value that must be broadcast using [sendrawtransaction](../../basic-docs/komodo-api/rawtransactions.html#sendrawtransaction).

The `sendrawtransaction` method will then return a `txid`, which is later used in the [rewardsunlock](../../basic-docs/cryptoconditions/cc-rewards.html#rewardsunlock) method. In general, it is best to save this `txid` in a secure location.

If the final `txid` is lost, it is possible to find it again. See [rewardsunlock](../../basic-docs/cryptoconditions/cc-rewards.html#rewardsunlock) for more information.
...
The method returns a hex value which must then be broadcast using the [sendrawtransaction](../../basic-docs/komodo-api/rawtransactions.html#sendrawtransaction) method to complete the command.

If you attempt to unlock your funds before the minimum period is met, the daemon returns this error:

The method requires the `txid` that was returned as a result of the original [rewardslock](../../basic-docs/cryptoconditions/cc-rewards.html#rewardslock) method.

If the original `txid` is lost, it is possible to find it again by either rebroadcasting the original `hex` (if it is available), or by scanning through available utxos using the [getaddressutxos](../../basic-docs/komodo-api/address.html#getaddressutxos) method.
