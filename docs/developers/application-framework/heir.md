...
[tmpdoc](https://developers.komodoplatform.com/basic-docs/cryptoconditions/cc-heir.html)

[code @ github](https://github.com/KomodoPlatform/komodo/blob/master/src/cc/heir.cpp)
...
The [heiradd](../../basic-docs/cryptoconditions/cc-heir.html#heiradd) method allows anyone, including users who are neither the owner nor the heir, to add funds to the address. These additions are considered donations and they do not affect the `inactivitytime` calculations that can unlock the funds for the heir. The `heiradd` method warns the user that they are making a donation if the method detects that the user is neither the owner nor the heir.

The Heir CC module accepts both coins and tokens. These can be the base coin of the asset chain, on-chain tokens created via the [Tokens](../../basic-docs/cryptoconditions/cc-tokens.html) CC module that represent on-chain assets, or even tokens that are formed via the [Gateways](../../basic-docs/cryptoconditions/cc-gateways.html) module to represent off-chain assets or other cryptocurrencies.

### Flow of the Heir CC Module:

- Anyone can create a new Heir CC address using [heirfund](../../basic-docs/cryptoconditions/cc-heir.html#heirfund)
- The owner of this address can add more funds using [heiradd](../../basic-docs/cryptoconditions/cc-heir.html#heiradd)
- Any other user, including the heir, can add donations using [heiradd](../../basic-docs/cryptoconditions/cc-heir.html#heiradd)
- The owner can claim funds at any time using [heirclaim](../../basic-docs/cryptoconditions/cc-heir.html#heirclaim) 
- Once `inactivitytime` period is reached, the heir can also claim funds using [heirclaim](../../basic-docs/cryptoconditions/cc-heir.html#heirclaim)
- To retrieve the details of a particular funding plan, use [heirinfo](../../basic-docs/cryptoconditions/cc-heir.html#heirinfo)
- To retrieve a list of all funding plans on the asset chain, use [heirlist](../../basic-docs/cryptoconditions/cc-heir.html#heirlist)
- To output Heir CC addresses, use [heiraddress](../../basic-docs/cryptoconditions/cc-heir.html#heiraddress)
...
::: tip
The following example demonstrates Heir CC usage when utilizing coins, instead of tokens. When instead using tokens, create the tokens first with the [tokencreate](../../basic-docs/cryptoconditions/cc-tokens.html#tokencreate) method and pass the `tokenid` as the last parameter of [heirfund](../../basic-docs/cryptoconditions/cc-heir.html#heirfund).
:::
...
::: tip
Use the [<b>heirlist</b>](../../basic-docs/cryptoconditions/cc-heir.html#heirlist) method to find a <b>fundingtxid</b>. 
:::
...
| fundingtxid | (string) | the transaction id returned from the original [heirfund](../../basic-docs/cryptoconditions/cc-heir.html#heirfund) transaction                                                                       |
...
| Structure             | Type      | Description                                                                                                                                   |
| --------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| fundingtxid | (string) | the id of the funding plan, the txid of [heirfund](../../basic-docs/cryptoconditions/cc-heir.html#heirfund) transaction |
...
| myaddress                 | (string) | the unmodified normal public address of the pubkey [used to launch the daemon](../../basic-docs/cryptoconditions/cryptoconditions-instructions.html#creating-and-launching-with-a-pubkey). This is the normal address used to withdraw funds in coins from from HeirCC`1of2`Address. This property is applicable to any user who wants to use the Heir CC plan (owner or heir)                                                     |
